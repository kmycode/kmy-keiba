  ...
  static class DatabasePresetModel
  {
    ...
    public static IReadOnlyList<FinderColumnDefinition<FinderRaceHorseItem>> GetFinderRaceHorseColumns()
    {
      return new[]
      {
        ...
        FinderColumnDefinition.Create<FinderRaceHorseItem>(2, FinderColumnType.NumericText, 40, "PCI", h => h.Pci, v => ((double)v) != default ? ((double)v).ToString("N1") : string.Empty, (h, v) => AnalysisUtil.CompareValue((double)v, 55, 45)),
        FinderColumnDefinition.Create<FinderRaceHorseItem>(2, FinderColumnType.NumericText, 40, "PCI3", h => h.Pci3, v => ((double)v) != default ? ((double)v).ToString("N1") : string.Empty, (h, v) => AnalysisUtil.CompareValue((double)v, 55, 45)),
        FinderColumnDefinition.Create<FinderRaceHorseItem>(2, FinderColumnType.NumericText, 40, "RPCI", h => h.Rpci, v => ((double)v) != default ? ((double)v).ToString("N1") : string.Empty, (h, v) => AnalysisUtil.CompareValue((double)v, 55, 45)),
