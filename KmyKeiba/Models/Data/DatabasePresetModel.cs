  ...
  static class DatabasePresetModel
  {
    ...
    public static IReadOnlyList<FinderColumnDefinition<FinderRaceHorseItem>> GetFinderRaceHorseColumns()
    {
      return new[]
      {
        ...
        FinderColumnDefinition.Create<FinderRaceHorseItem>(2, FinderColumnType.NumericText, 40, "前3H", h => h.Analyzer.Race.BeforeHaronTime3 / 10.0, v => ((double)v) != default ? v : string.Empty, (h, v) => AnalysisUtil.CompareValue((double)v, 35, 45, true)),
        FinderColumnDefinition.Create<FinderRaceHorseItem>(2, FinderColumnType.NumericText, 40, "前4H", h => h.Analyzer.Race.BeforeHaronTime4 / 10.0, v => ((double)v) != default ? v : string.Empty, (h, v) => AnalysisUtil.CompareValue((double)v, 45, 55, true)),
        FinderColumnDefinition.Create<FinderRaceHorseItem>(2, FinderColumnType.NumericText, 40, "後3H", h => h.Analyzer.Race.AfterHaronTime3 / 10.0, v => ((double)v) != default ? v : string.Empty, (h, v) => AnalysisUtil.CompareValue((double)v, 35, 45, true)),
        FinderColumnDefinition.Create<FinderRaceHorseItem>(2, FinderColumnType.NumericText, 40, "後4H", h => h.Analyzer.Race.AfterHaronTime4 / 10.0, v => ((double)v) != default ? v : string.Empty, (h, v) => AnalysisUtil.CompareValue((double)v, 45, 55, true)),
        FinderColumnDefinition.Create<FinderRaceHorseItem>(2, FinderColumnType.NumericText, 40, "PCI", h => h.Pci, v => ((double)v) != default ? ((double)v).ToString("N1") : string.Empty, (h, v) => AnalysisUtil.CompareValue((double)v, 45, 55)),
